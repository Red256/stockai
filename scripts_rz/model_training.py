...
from scripts_rz.auto_arima import train_autoarima_for_batch
from scripts_rz.generate_ticker_list import get_ticker_list
from scripts_rz.trade_rsi_strategy import create_position,  implement_rsi_strategy


def load_performance(model_type="RSI"):
    output_file = f"{SCRIPTS_PATH}/Ticker_performance_{model_type.lower()}.csv"
...
def rsi_model():
    ...
    output_file = f"{SCRIPTS_PATH}/Ticker_performance_rsi.csv"
...
def arima_model():
    # let create a performance file under root folder
    output_file = f"{SCRIPTS_PATH}/Ticker_performance_arima.csv"
    # files headers: ["ticker", "interval", "price_type", "starting", "ending", "performance"]
    # to simplify, every time we retrain, simply overwrite.
    # add trained time at the end
    interval = '1d'
