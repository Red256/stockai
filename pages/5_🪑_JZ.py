# %%writefile /content/drive/MyDrive/StocksFolder/scripts_jz/📈_AC.py
...
#scripts folder: StocksFolder/scripts_jz. here StocksFolder/scripts_jz

_SCRIPTS_FOLDER = "scripts_jz"
PROJECT_PATH = os.getcwd()
DATA_PATH = f"{PROJECT_PATH}/{_SCRIPTS_FOLDER}/data"
SCRIPTS_PATH = f"{PROJECT_PATH}/scripts_jz"
sys.path.append(PROJECT_PATH)

jz_API_KEY = 'jz_API_KEY'
jz_API_SECRET = 'jz_API_SECRET'
jz_END_POINT = 'jz_END_POINT'

from alpaca.trading.client import TradingClient
import alpaca_trade_api as tradeapi

from scripts_jz.generate_ticker_list import choose_and_save_my_list, get_ticker_list
from scripts_jz.get_histories import download_histories, get_one_ticker_df

from scripts_jz.ticker_eda import visualize_sma_one_ticker, visualize_ewm_one_ticker
from scripts_jz.rs_rsi import plot_RSI, plot_RSI_streamlit

from scripts_jz.trade_rsi_strategy import plot_trading_points, create_plot_position

from scripts_jz.auto_arima import (
        plot_stock,
        test_stationarity,
...
from scripts_jz.model_training import load_performance, rsi_model, arima_model, get_candidates
...
def get_tradeclient_api():
    jz_API_KEY = st.session_state["jz_API_KEY"]
    jz_API_SECRET = st.session_state["jz_API_SECRET"]
    jz_END_POINT = st.session_state["jz_END_POINT"]

    trading_client = TradingClient(jz_API_KEY, jz_API_SECRET, paper=True)
    api =  tradeapi.REST(jz_API_KEY,jz_API_SECRET, jz_END_POINT)
...
with tabs[4]:
    ...
    if "jz_API_KEY" not in st.session_state:
...
with tabs[5]:
    ...
    if "jz_API_KEY" in st.session_state:
        st.markdown("alpaca api key and secret have already been loaded")
        reload = st.button("re-load/refresh api key/secret")
        if reload:
            del st.session_state["jz_API_KEY"]
            del st.session_state["jz_API_SECRET"]
            del st.session_state["jz_END_POINT"]
    else:
        ...
        if key_file is not None:
            ...
            has_all_info = 0
            if "jz_API_KEY" in key_file_json:
                API_KEY = key_file_json["jz_API_KEY"]
                st.session_state["jz_API_KEY"] = API_KEY
                has_all_info += 1
            if "jz_API_SECRET" in key_file_json:
                API_SECRET = key_file_json["jz_API_SECRET"]
                st.session_state["jz_API_SECRET"] = API_SECRET
                has_all_info += 1
            if "jz_END_POINT" in key_file_json:
                END_POINT = key_file_json["jz_END_POINT"]
                st.session_state["jz_END_POINT"] = END_POINT
                has_all_info += 1
