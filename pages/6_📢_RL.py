# %%writefile /content/drive/MyDrive/StocksFolder/scripts_rl/📈_AC.py
...
#scripts folder: StocksFolder/scripts_rl. here StocksFolder/scripts_rl

_SCRIPTS_FOLDER = "scripts_rl"
PROJECT_PATH = os.getcwd()
DATA_PATH = f"{PROJECT_PATH}/{_SCRIPTS_FOLDER}/data"
SCRIPTS_PATH = f"{PROJECT_PATH}/scripts_rl"
sys.path.append(PROJECT_PATH)

rl_API_KEY = 'rl_API_KEY'
rl_API_SECRET = 'rl_API_SECRET'
rl_END_POINT = 'rl_END_POINT'

from alpaca.trading.client import TradingClient
import alpaca_trade_api as tradeapi

from scripts_rl.generate_ticker_list import choose_and_save_my_list, get_ticker_list
from scripts_rl.get_histories import download_histories, get_one_ticker_df

from scripts_rl.ticker_eda import visualize_sma_one_ticker, visualize_ewm_one_ticker
from scripts_rl.rs_rsi import plot_RSI, plot_RSI_streamlit

from scripts_rl.trade_rsi_strategy import plot_trading_points, create_plot_position

from scripts_rl.auto_arima import (
        plot_stock,
        test_stationarity,
...
from scripts_rl.model_training import load_performance, rsi_model, arima_model, get_candidates
...
def get_tradeclient_api():
    rl_API_KEY = st.session_state["rl_API_KEY"]
    rl_API_SECRET = st.session_state["rl_API_SECRET"]
    rl_END_POINT = st.session_state["rl_END_POINT"]

    trading_client = TradingClient(rl_API_KEY, rl_API_SECRET, paper=True)
    api =  tradeapi.REST(rl_API_KEY,rl_API_SECRET, rl_END_POINT)
...
with tabs[4]:
    ...
    if "rl_API_KEY" not in st.session_state:
...
with tabs[5]:
    ...
    if "rl_API_KEY" in st.session_state:
        st.markdown("alpaca api key and secret have already been loaded")
        reload = st.button("re-load/refresh api key/secret")
        if reload:
            del st.session_state["rl_API_KEY"]
            del st.session_state["rl_API_SECRET"]
            del st.session_state["rl_END_POINT"]
    else:
        ...
        if key_file is not None:
            ...
            has_all_info = 0
            if "rl_API_KEY" in key_file_json:
                API_KEY = key_file_json["rl_API_KEY"]
                st.session_state["rl_API_KEY"] = API_KEY
                has_all_info += 1
            if "rl_API_SECRET" in key_file_json:
                API_SECRET = key_file_json["rl_API_SECRET"]
                st.session_state["rl_API_SECRET"] = API_SECRET
                has_all_info += 1
            if "rl_END_POINT" in key_file_json:
                END_POINT = key_file_json["rl_END_POINT"]
                st.session_state["rl_END_POINT"] = END_POINT
                has_all_info += 1
