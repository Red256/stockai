# %%writefile /content/drive/MyDrive/StocksFolder/scripts_al/📈_AC.py
...
#scripts folder: StocksFolder/scripts_al. here StocksFolder/scripts_al

_SCRIPTS_FOLDER = "scripts_al"
PROJECT_PATH = os.getcwd()al_
DATA_PATH = f"{PROJECT_PATH}/{_SCRIPTS_FOLDER}/data"
SCRIPTS_PATH = f"{PROJECT_PATH}/scripts_al"
sys.path.append(PROJECT_PATH)

al_API_KEY = 'al_API_KEY'
al_API_SECRET = 'al_API_SECRET'
al_END_POINT = 'al_END_POINT'

from alpaca.trading.client import TradingClient
import alpaca_trade_api as tradeapi

from scripts_al.generate_ticker_list import choose_and_save_my_list, get_ticker_list
from scripts_al.get_histories import download_histories, get_one_ticker_df

from scripts_al.ticker_eda import visualize_sma_one_ticker, visualize_ewm_one_ticker
from scripts_al.rs_rsi import plot_RSI, plot_RSI_streamlit

from scripts_al.trade_rsi_strategy import plot_trading_points, create_plot_position

from scripts_al.auto_arima import (
        plot_stock,
        test_stationarity,
...
from scripts_al.model_training import load_performance, rsi_model, arima_model, get_candidates
...
def get_tradeclient_api():
    al_API_KEY = st.session_state["al_API_KEY"]
    al_API_SECRET = st.session_state["al_API_SECRET"]
    al_END_POINT = st.session_state["al_END_POINT"]

    trading_client = TradingClient(al_API_KEY, al_API_SECRET, paper=True)
    api =  tradeapi.REST(al_API_KEY,al_API_SECRET, al_END_POINT)
...
with tabs[4]:
    ...
    if "al_API_KEY" not in st.session_state:
...
with tabs[5]:
    ...
    if "al_API_KEY" in st.session_state:
        st.markdown("alpaca api key and secret have already been loaded")
        reload = st.button("re-load/refresh api key/secret")
        if reload:
            del st.session_state["al_API_KEY"]
            del st.session_state["al_API_SECRET"]
            del st.session_state["al_END_POINT"]
    else:
        ...
        if key_file is not None:
            ...
            has_all_info = 0
            if "al_API_KEY" in key_file_json:
                API_KEY = key_file_json["al_API_KEY"]
                st.session_state["al_API_KEY"] = API_KEY
                has_all_info += 1
            if "al_API_SECRET" in key_file_json:
                API_SECRET = key_file_json["al_API_SECRET"]
                st.session_state["al_API_SECRET"] = API_SECRET
                has_all_info += 1
            if "al_END_POINT" in key_file_json:
                END_POINT = key_file_json["al_END_POINT"]
                st.session_state["al_END_POINT"] = END_POINT
                has_all_info += 1
