...
from scripts_rz.ticker_eda import visualize_sma_one_ticker, visualize_ewm_one_ticker
from scripts_rz.rs_rsi import plot_RSI, plot_RSI_streamlit

from scripts_rz.trade_rsi_strategy import plot_trading_points, create_plot_position

from scripts_rz.auto_arima import (
        plot_stock,
        test_stationarity,
...
with tabs[5]:
    ...
    if MY_API_KEY in st.session_state:
        ...
        if key_file is not None:
            ...
            has_all_info = 0
            if MY_API_KEY in key_file_json:
                API_KEY = key_file_json[MY_API_KEY]
                st.session_state.TEMPLATE_API_KEY = API_KEY
                has_all_info += 1
            if MY_API_SECRET in key_file_json:
                API_SECRET = key_file_json[MY_API_SECRET]
                st.session_state.TEMPLATE_API_SECRET = API_SECRET
                has_all_info += 1
            if MY_END_POINT in key_file_json:
                END_POINT = key_file_json[MY_END_POINT]
